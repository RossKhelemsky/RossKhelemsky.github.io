...
Then I run OLS multiple linear regression, where monthly gas consumption($\mathbf{G}$) is dependent on the real price of gas($\mathbf{P_GR}$) and real monthly income($\mathbf{I_R}$). This fits a plane to the data, minimizing the sum of squared residuals, in the form: 
